/**
 * Miyabara_MagentoAI — image modification widget
 */
define([
    'jquery',
    ...
    $.widget('mage.mageAiImageModify', {
        ...
        /**
         * Inject "Edit Image with MagentoAI" after the generate button (or add-video). Idempotent.
         */
        _injectButton: function () {
            if ($('#' + this.options.buttonId).length) { return; }

            var $anchor = $('#' + this.options.generateButtonId);
            if (!$anchor.length) { $anchor = $('#add_video_button'); }
            ...
            var $btn = $('<button>', {
                ...
                title:   $.mage.__('Edit Image with MagentoAI')
            }).html('<span>' + $.mage.__('Edit Image with MagentoAI') + '</span>');
        ...
        /**
         * Bind all in-modal click events once.
         *
         * @param {jQuery} $modal
         */
        ...
        /**
         * Switch the active modal view: 'list', 'prompt', or 'result'.
         *
         * @param {jQuery} $modal
         * @param {string} name
         */
        ...
        /**
         * Populate the image grid with the current product gallery images.
         *
         * @param {jQuery} $modal
         */
        _renderImageList: function ($modal) {
            var $grid  = $modal.find('[data-role=modify-grid]').empty();
            var $empty = $modal.find('[data-role=modify-empty]');

            this._galleryImages = this._collectGalleryImages();

            if (!this._galleryImages.length) { $empty.show(); return; }
            $empty.hide();
            ...
            $.each(this._galleryImages, function (index, data) {
                ...
                $('<button>', {
                    ...
                    'data-index':  index
                }).html('<span>' + $.mage.__('Edit with MagentoAI') + '</span>').appendTo($card);
                $card.appendTo($grid);
            });
        },

        /**
         * Collect non-removed, non-video images from the product gallery.
         *
         * @returns {Array}
         */
        _collectGalleryImages: function () {
            ...
            this._getGallery().find('[data-role=image]').each(function () {
                var $el  = $(this);
                var data = $el.data('imageData');
                if ($el.hasClass('removed') || !data || !data.file) { return; }
                if (data['media_type'] && data['media_type'] !== 'image') { return; }
                images.push(data);
            });
            return images;
        },
        ...
        _getGallery: function () {
            var $g = $(this.options.gallerySelector).first();
            return $g.length ? $g : $('#media_gallery_content');
        },

        /**
         * Send the selected image and prompt to the modify controller.
         *
         * @param {jQuery} $modal
         */
        _modify: function ($modal) {
            var self          = this;
            var prompt        = $modal.find('#mp-modify-prompt').val().trim();
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.error) {
                        alert({ title: $.mage.__('Image Edit Error'), content: response.data });
                        return;
                    }
        ...
        /**
         * Replace the original gallery image with the modified version, preserving roles and position.
         *
        ...
        _replaceInGallery: function (oldData, newData) {
            ...
            var inst     = $gallery.data('mage-productGallery');
            var roles    = [];
            var oldIndex = -1;
            ...
            if (inst && inst.options && inst.options.types) {
                $.each(inst.options.types, function (code, type) {
                    if (type && type.value === oldData.file) { roles.push(code); }
                });
